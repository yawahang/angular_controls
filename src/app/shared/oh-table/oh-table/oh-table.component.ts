...
import { MvNavigationActionList } from '../oh-table.model';
import {
  ExcelExportService,
  MvExcelExportOption,
} from '../excel-export.service';
import { Subject } from 'rxjs';
import { MvOhTableColumn, MvOhTableConfig } from './oh-table.model';

@Component({
  selector: 'oh-table',
  templateUrl: './oh-table.component.html',
  styleUrls: ['./oh-table.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class OhTableComponent implements OnInit, OnDestroy {
  ...
  @Input('config') set config(conf: MvOhTableConfig) {
    if (conf) {
      ...
      const stickyCol = this.gridColumns.filter(
        (col: MvOhTableColumn) => col.sticky
      );
      ...
      const disableSort = this.gridColumns.filter(
        (col: MvOhTableColumn) => col.disableSort
      );
      ...
      this.actionColumns = this.gridColumns
        .filter((col: MvOhTableColumn) => col.type === 'action')
        .map((col: MvOhTableColumn) => col.name);
      if (this.displayedColumns.length === 0) {
        // affects column order if re-assigned
        this.displayedColumns = this.gridColumns.map(
          (col: MvOhTableColumn) => col.name
        );
      }

      this.checkColumns = this.gridColumns
        .filter((col: MvOhTableColumn) => col.type === 'checkBox')
        .map((col: MvOhTableColumn) => col.name);
  ...
  gridColumns: MvOhTableColumn[] = [];
  dataSource!: MatTableDataSource<any>;
  gridData: any[] = [];

  totalRows: number | undefined;
  selection: SelectionModel<any> | undefined;
  gridConfig: MvOhTableConfig | undefined;
